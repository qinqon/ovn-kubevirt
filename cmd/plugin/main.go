...
func (j *JoinRouter) routeDynamicAddressToGw(ctx *CmdContext, lsp *nbdb.LogicalSwitchPort) error {
	...
	policyPredicate := func(item *nbdb.LogicalRouterPolicy) bool {
		return item.Match == keepInternalIPsNexthop.Match && item.Action == keepInternalIPsNexthop.Action
	}
	...
	routeToGw := nbdb.LogicalRouterPolicy{
		...
		Nexthop: &nodeGwAddress,
	}

	policyPredicate = func(item *nbdb.LogicalRouterPolicy) bool {
		return item.Match == keepInternalIPsNexthop.Match && item.Action == keepInternalIPsNexthop.Action && item.Nexthop != nil && *item.Nexthop == *routeToGw.Nexthop
	}
